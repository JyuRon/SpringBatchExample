...
public class ParallelConfiguration {
    ...
    public ParallelConfiguration(JobBuilderFactory jobBuilderFactory,
    ...
    public Job userJob() throws Exception {
            return this.jobBuilderFactory.get(JOB_NAME)
                    ...
                    .next(this.splitFlow(null, null))
                    .build()
                    .build();
    }

    //TODO: Parallel 적용
   @Bean(JOB_NAME + "_saveUsrFlow")
    ...
    public Flow splitFlow(@Value("#{jobParameters[date]}") String date,
        ...
        return new FlowBuilder<SimpleFlow>(JOB_NAME + "_splitFlow")
                ...
                .add(userLevelUpFlow, orderStatisticsFlow(date,path))
                .build();
    ...
    //TODO: Parallel 적용
    private Flow orderStatisticsFlow(String date, String path) throws Exception {
        return new FlowBuilder<SimpleFlow>(JOB_NAME + "_orderStatisticsFlow")
                .start(new JobParametersDecide("date"))  // decide함수가 상황에 맞게 return을 한다.
                .on(JobParametersDecide.CONTINUE.getName()) // decide함수가 return을 한 값이 CONTINUE인지를 검사한다.
                .to(this.orderStatisticsStep(date,path))// 맞으면 해당 Step을 실행한다.
                .build();
    }

    private Step orderStatisticsStep(@Value("#{jobParameters[date]}") String date,
